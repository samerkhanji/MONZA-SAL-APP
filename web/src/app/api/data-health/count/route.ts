...
export async function GET() {
  ...
  const carSelect =
    "id, vin, model, status, location_type, exterior_color, interior_color, engine_number, date_arrived, battery_percent, warranty_vehicle_dms, warranty_vehicle_expiry, warranty_battery_expiry, reservation_date, reserved_by, delivery_date, software_version, software_update, dongle, issue";
  ...
  const [
    ...
    supabase
      .from("sales_orders")
      .select(
        "id, car_id, customer_id, sale_date, date_bought, delivery_date, reservation_date"
      )
  ...
  const soldCarsNoOrder = carsList.filter((c) => ["sold", "reserved"].includes(String(c.status)) && !soCarIds.has(c.id));
  ...
  const carsMissingData = carsList.filter((c) => empty(c.vin) || empty(c.model) || empty(c.exterior_color) || empty(c.interior_color) || empty(c.engine_number) || empty(c.location_type) || empty(c.warranty_vehicle_expiry) || empty(c.warranty_battery_expiry) || c.battery_percent == null || empty(c.date_arrived));
  const soMissingData = soList.filter((s) => empty(s.car_id) || empty(s.customer_id) || empty(s.sale_date));
  ...
  const carsWarrantyMissingSold = carsWarrantyMissing.filter((c) => ["sold", "reserved"].includes(String(c.status)));
  ...
  const deliveredNoDate = carsList.filter((c) => c.status === "sold" && empty(c.delivery_date));
  ...
  // Parity with the Data Health page: only flag outdated software when a real
  // target version exists (placeholder "latest" → no comparison, always empty).
  const hasRealSoftwareTarget = Object.values(LATEST_SOFTWARE_VERSION_BY_MODEL).some((t) => t !== "latest");
  const carsOutdatedSoftware = !hasRealSoftwareTarget
    ? []
    : carsList.filter((c) => {
        const v = c.software_version;
        if (empty(v)) return false;
        const model = String(c.model ?? "").trim() || "Voyah";
        const target =
          LATEST_SOFTWARE_VERSION_BY_MODEL[model] ??
          LATEST_SOFTWARE_VERSION_BY_MODEL["Voyah"] ??
          "latest";
