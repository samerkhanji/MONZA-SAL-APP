...
export function CarsInventoryClient({
  initialCars,
}: {
  initialCars: CarDisplay[];
}) {
  ...
  // Initial inventory is rendered server-side (see page.tsx). Only fetch on
  // mount when the server returned nothing — that covers a genuinely empty
  // list or a server-side fetch error, so the client can surface it.
  useEffect(() => {
    if (initialCars.length === 0) fetchCars();
  }, [fetchCars, initialCars]);
