-- 124_grn_writes_part_movements.sql
-- When a GRN receipt bumps part stock (good/extra conditions), also write a
-- corresponding row into public.part_movements so the inbound stock change has
-- the same audit trail as use_part_on_job / apply_part_to_job stock-outs.
--
-- part_movements columns (verified via information_schema):
  ...
  LOOP
    ...
    IF v_cond IN ('good', 'extra') AND v_qty > 0 THEN
      ...
      IF v_part_id IS NOT NULL THEN
        ...
        -- Audit trail: record the inbound stock movement.
        INSERT INTO public.part_movements (
          part_id, movement_type, quantity, note, created_by
        ) VALUES (
          v_part_id, 'stock_in', v_qty::integer,
          'GRN ' || p_grn_number, v_caller
        );
      END IF;
    END IF;
  END LOOP;
