-- emit_notification: single entry point for any code path that needs to
-- raise a notification. Resolves notification_event_rules → recipients →
-- inserts one notifications row per recipient, respecting:
--   - user preferences (in_app_enabled, email_enabled, etc.)
--   - mute_entity_keys (skip if user muted this related_entity_type:id)
-- Email/WhatsApp delivery: in-app row always inserted; future channel
-- workers consume rows where delivered_*_at IS NULL and the rule asks
-- for that channel.
    ...
    v_recipient_ids := CASE v_rule.recipient_kind
      ...
      WHEN 'role' THEN (
        SELECT array_agg(id) FROM public.profiles
         WHERE user_role::text = v_rule.recipient_value AND is_active = true
      )
