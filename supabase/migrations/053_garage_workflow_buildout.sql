-- ============================================
-- MONZA CRM - Garage workflow buildout
-- Migration 053
--
-- Adds the connective tissue between bays, jobs, parts, and time entries:
--   - Bay status state machine (empty | occupied | cleaning | maintenance)
--   - bay_assignment_history audit log
--   - Snapshot columns on job_parts (price at time of use)
--   - bay_entered_at / bay_exited_at on garage_jobs
--   - 4 SECURITY DEFINER RPCs:
--       scan_vin_to_bay, release_bay,
--       apply_part_to_job, return_part_from_job
--   - Trigger that auto-recomputes garage_jobs.actual_hours from
--     job_time_entries.duration_minutes
--   - 3 invoker-rights views:
--       garage_job_efficiency, garage_bay_utilization,
--       garage_employee_efficiency
--
-- Verified end-to-end against live data via Supabase MCP before commit.
-- ============================================
